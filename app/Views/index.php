<?php
$allSurprises = is_array($allSurprises ?? null) ? $allSurprises : [];
$surprisesLeftValue = is_numeric($surprisesLeft ?? null) ? (int) $surprisesLeft : 0;
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <main class="h-screen flex flex-col overflow-hidden">
        <div id="intro" class="absolute w-full h-screen bg-purple-700 sticky top-0 items-center justify-center flex">
            <div class="text-4xl text-white flex flex-col md:flex-row items-center justify-center gap-3">
                ...
                <span class="text-pink-300 text-5xl"><?= e((string) $surprisesLeftValue) ?></span>
                <span>surprise<?= $surprisesLeftValue === 1 ? '' : 's' ?> left<?= $surprisesLeftValue === 0 ? '...' : ' to go!' ?></span>
            </div>
        ...
        <div id="surprises" class="transition-all duration-1000 hidden opacity-0 m-auto max-w-screen-2xl w-full flex flex-wrap gap-8 sm:gap-12 p-6 justify-center items-center overflow-auto">
            ...
            <?php for ($i = 1; $i <= 30; $i++): ?>
                <?php
                $s = $allSurprises[$i - 1] ?? null;
                $sCardClass = 'surprise-card relative w-40 h-40 sm:w-48 sm:h-48 border-2 border-gray-300 rounded-lg cursor-pointer overflow-hidden';
                $iconClass = 'absolute w-full h-full text-6xl sm:text-8xl text-gray-500 flex items-center justify-center';
                ?>

                <?php if ($s === null || ((empty($s['viewed']) && empty($s['revealDate'])))): ?>
                    <div class="<?= e($sCardClass) ?> upcoming" onclick="handleClick(this)">
                        <div class="<?= e($iconClass) ?> opacity-40"><?= e((string) $i) ?></div>
                    </div>
                <?php else: ?>
                    <?php
                    $stateClass = !empty($s['viewed']) ? 'viewed' : (!empty($s['revealDate']) ? 'upcoming' : '');
                    $displayRevealDate = '';
                    if (!empty($s['revealDate'])) {
                        try {
                            $displayRevealDate = (new DateTimeImmutable((string) $s['revealDate']))
                    ...
                    ?>
                    <div
                        class="<?= e(trim($sCardClass . ' ' . $stateClass)) ?>"
                        data-reveal-date="<?= e((string) ($s['revealDate'] ?? '')) ?>"
                        data-id="<?= e((string) ($s['id'] ?? '')) ?>"
                        onclick="handleClick(this)"
                    >
                        <div class="<?= e($iconClass) ?> icon-container transition-all duration-1000 <?= !empty($s['viewed']) ? 'opacity-10' : 'opacity-30' ?>">
                            <i class="<?= e((string) ($s['iconClass'] ?? '')) ?>"></i>
                        </div>
                        <div class="surprise-body w-full h-full relative p-2 pt-10">
                            <div class="absolute p-2 left-0 top-0 w-full flex justify-between items-center">
                                <span data-magnitude="<?= e((string) ($s['magnitude'] ?? '')) ?>" class="magnitude rounded bg-white text-black px-2"><?= e((string) ($s['magnitude'] ?? '')) ?></span>
                                <span class="text-sm text-gray-500"><?= e((string) $i) ?></span>
                                <span data-variety="<?= e((string) ($s['variety'] ?? '')) ?>" class="variety rounded bg-white text-black px-2 text-xs sm:font-semibold sm:tracking-wider"><?= e((string) ($s['variety'] ?? '')) ?></span>
                            </div>
                            <h1 class="transition-all duration-1000 text-base sm:text-lg <?= !empty($s['viewed']) ? 'opacity-100' : 'opacity-0' ?> mb-1 sm:mb-2 font-extrabold"><?= e((string) ($s['title'] ?? '')) ?></h1>
                            <p class="transition-all duration-1000 text-xs sm:text-base overflow-auto h-24 <?= !empty($s['viewed']) ? 'opacity-100' : 'opacity-0' ?>"><?= e((string) ($s['description'] ?? '')) ?></p>
                            <?php if (empty($s['viewed']) && !empty($s['revealDate'])): ?>
                                <div class="countdown-container absolute w-full h-40 text-gray-500 text-lg sm:text-xl text-center bottom-0 left-0">
                                    ...
                                    <div class="transition-all duration-1000 opacity-0 reveal-date absolute w-full bottom-0 text-base"><?= e($displayRevealDate) ?></div>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
